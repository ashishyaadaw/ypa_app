            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap relative">
                        ...
                        <tbody id="mentor-table-body" class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            {{-- fetch data --}}
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        ...
        function printTableBody(startCount, endCount, newData) {
            ...
            for (i; i < totalCount && i < endCount; i++) {
                ...
                $('#mentor-table-body').append(`<tr row-id="${element.id}" class="text-gray-700 w-full dark:text-gray-400 mentor-row">
        ...
        async function EditMentorRow(id) {
            var keys;
            var values;
            var dataToUpdate;
            await fetch('http://127.0.0.1:8000/api/mentors/' + id)
                .then(response => {
                    if (!response.ok) {
        ...
        async function DeleteMentor(id) {
            ...
            await fetch(`http://127.0.0.1:8000/api/mentors/${id}`, {
                ...
                .then(data => {
                    alertMessage('success', 'One mentor deleted');
        ...
        async function DeleteMentors(data) {
            // var data = {
            //     "title": "NOTICE HERE 4",
            //     "name": "DESCRIPTION",
            //     "date": "2024-10-13",
            ...
            await fetch(`http://127.0.0.1:8000/api/mentors`, {
                ...
                .then(data => {
                    alertMessage('success', 'Selected Mentors deleted!');
        ...
        async function UpdateMentor(data) {
            ...
            // "status": "/mentor/id"
            // };
            await fetch(`http://127.0.0.1:8000/api/mentors/${data.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
                ...
                .then(data => {
                    alertMessage('success', 'Mentor updated successfully!');
        ...
        async function CreateMentor(data) {
            ...
            await fetch(`http://127.0.0.1:8000/api/mentors`, {
                ...
                .then(data => {
                    ...
                    PrintMentor(0);
                    alertMessage('info', 'Please update mentor data!');
        ...
        async function PrintMentor(index) {
            await fetch('http://127.0.0.1:8000/api/mentors/')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
...
@push('script')
    ...
    $('.components > ul > li:nth-child(9)').css('color', '#9333EA');
@endpush
