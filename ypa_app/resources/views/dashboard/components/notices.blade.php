            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap relative">
                        ...
                        <tbody id="notice-table-body" class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            {{-- fetch data --}}
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        ...
        function printTableBody(startCount, endCount, newData) {
            ...
            for (i; i < totalCount && i < endCount; i++) {
                ...
                $('#notice-table-body').append(`<tr row-id="${element.id}" class="text-gray-700 w-full dark:text-gray-400 notice-row">
        ...
        async function EditNoticeRow(id) {
            ...
            var values;
            var dataToUpdate;
            await fetch('http://127.0.0.1:8000/api/notices/' + id)
                .then(response => {
                    if (!response.ok) {
        ...
        async function DeleteNotice(id) {
            ...
            await fetch(`http://127.0.0.1:8000/api/notices/${id}`, {
                ...
                .then(data => {
                    alertMessage('success', 'One notice deleted');
        ...
        async function DeleteNotices(data) {
            // var data = {
            //     "title": "NOTICE HERE 4",
            //     "name": "DESCRIPTION",
            //     "date": "2024-10-13",
            ...
            await fetch(`http://127.0.0.1:8000/api/notices`, {
                ...
                .then(data => {
                    alertMessage('success', 'Selected Notices deleted!');
        ...
        async function UpdateNotice(data) {
            ...
            // "status": "/notice/id"
            // };
            await fetch(`http://127.0.0.1:8000/api/notices/${data.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
                ...
                .then(data => {
                    alertMessage('success', 'Notice updated successfully!');
        ...
        async function CreateNotice(data) {
            ...
            await fetch(`http://127.0.0.1:8000/api/notices`, {
                ...
                .then(data => {
                    ...
                    PrintNotice(0);
                    alertMessage('info', 'Please update notice data!');
        ...
        async function PrintNotice(index) {
            await fetch('http://127.0.0.1:8000/api/notices/')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
...
@push('script')
    ...
    $('.components > ul > li:nth-child(2)').css('color', '#9333EA');
@endpush
