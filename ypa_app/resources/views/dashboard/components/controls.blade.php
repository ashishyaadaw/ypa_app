...
@section('content')
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap relative">
                        ...
                        <tbody id="blog-table-body" class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            {{-- fetch data --}}
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        ...
        //     sampledata['category'] = formattedDate;
        // }
        ...
        function printTableBody(startCount, endCount, newData) {
            ...
            for (i; i < totalCount && i < endCount; i++) {
                ...
                $('#blog-table-body').append(`<tr row-id="${element.id}" class="text-gray-700 w-full dark:text-gray-400 blog-row">
        ...
        async function EditBlogRow(id) {
            var keys;
            var values;
            var dataToUpdate;
            await fetch('http://127.0.0.1:8000/api/controls/' + id)
                .then(response => {
                    if (!response.ok) {
        ...
        async function DeleteBlog(id) {
            ...
            await fetch(`http://127.0.0.1:8000/api/controls/${id}`, {
                ...
                .then(data => {
                    alertMessage('success', 'One blog deleted');
        ...
        async function DeleteControls(data) {
            // var data = {
            //     "title": "NOTICE HERE 4",
            //     "name": "DESCRIPTION",
            //     "category": "2024-10-13",
            ...
            //     "status": "/blog/id"
            // };
            await fetch(`http://127.0.0.1:8000/api/controls`, {
                    method: 'DELETE',
                    headers: {
                ...
                .then(data => {
                    alertMessage('success', 'Selected Controls deleted!');
        ...
        async function UpdateBlog(data) {
            ...
            // "status": "/blog/id"
            // };
            await fetch(`http://127.0.0.1:8000/api/controls/${data.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
                ...
                .then(data => {
                    alertMessage('success', 'Blog updated successfully!');
        ...
        async function CreateBlog(data) {
            ...
            await fetch(`http://127.0.0.1:8000/api/controls`, {
                ...
                .then(data => {
                    ...
                    PrintBlog(0);
                    alertMessage('info', 'Please update blog data!');
        ...
        async function PrintBlog(index) {
            await fetch('http://127.0.0.1:8000/api/controls/')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
...
@push('script')
    ...
    $('.components > ul > li:nth-child(6)').css('color', '#9333EA');
@endpush
