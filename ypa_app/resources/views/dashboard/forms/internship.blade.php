...
@section('content')
    <main class="h-full overflow-y-auto relative">
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap relative">
                        ...
                        <tbody id="internship-table-body" class="bg-white divide-y ">
                            {{-- fetch data --}}
                        </tbody>
                    </table>
                </div>
        ...
        <div
            class="mx-auto hidden pt-20 internship-update-form absolute w-full top-0 bg-slate-500 px-4  py-6 sm:px-6 sm:py-12 lg:px-8">
            ...
            <div class="p-10 pt-8">
                <h1 class="mb-8  font-bold font-raleway text-4xl">Internship Update Form</h1>
    ...
    <script>
        function PrintUpdateForm(sampledata) {
            ...
            forms.forEach(element => {
                ...
                if (element.type === 'file' && element.name === 'file_path') {
                    ...
                    <div class="mb-5">
                        ...
                        <img class="h-20" src="${element.placeholder}" alt="Your Image">
                            </div>
             </div>`);
                } else if (element.type === 'file' && element.name === 'file_path_resume') {
                    $('.forms-file-group').append(`             
                    <div class="mb-5">
                        <label for="${element.name}" class="mb-3 block text-base font-medium text-[#07074D]">
                        ${element.label}
                        ...
                        <a href="${element.placeholder}">
                              <div class="display_upload_${element.name} flex flex-row">
                        <img class="h-20" src="/cliparts/pdf.png" alt="Your Image"><span>${element.placeholder}</span>
                            </div></a>
             </div>`);
            ...
            $('#submit_internship_form').click(function(event) {
                ...
                //     "first_name": "John 2",
                //     "last_name": "Doe",
                //     "email": "john.doe@examaadscpdddlaggse.com",
                //     "phone_number": "123-456-7890",
                //     "dob": "1990-01-01",
                //     "address": "123 Main St",
                //     "city": "Anytown",
        ...
        $(document).ready(function() {
            ...
            $('.internship-update-form button[for="close-update-form"]').click(function() {
                PrintInternship(count)
                $(this).parent().hide();
            })
        })
    </script>
    <script>
        ...
        function printTableBody(startCount, endCount, newData) {
            ...
            for (i; i < totalCount && i < endCount; i++) {
                ...
                $('#internship-table-body').append(`<tr row-id="${element.id}" class="text-gray-700 w-full  internship-row">
        ...
        async function EditInternshipRow(id) {
            var keys;
            var values;
            var dataToUpdate;
            await fetch('http://127.0.0.1:8000/api/internships/' + id)
                .then(response => {
                    if (!response.ok) {
        ...
        async function DeleteInternship(id) {
            ...
            await fetch(`http://127.0.0.1:8000/api/internships/${id}`, {
                ...
                .then(data => {
                    alertMessage('success', 'One internship deleted');
        ...
        async function DeleteInternships(data) {
            // var data = {
            //     "title": "NOTICE HERE 4",
            //     "name": "DESCRIPTION",
            //     "date": "2024-10-13",
            ...
            await fetch(`http://127.0.0.1:8000/api/internships`, {
                ...
                .then(data => {
                    alertMessage('success', 'Selected Internships deleted!');
        ...
        async function UpdateInternship(data) {
            // var data = {
            // "id": "1",
            // "title": "NOTICE HERE 4",
            // "name": "DESCRIPTION",
            ...
            // "status": "/internship/id"
            // };
            await fetch(`http://127.0.0.1:8000/api/internships/${data.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
                ...
                .then(data => {
                    alertMessage('success', 'Internship updated successfully!');
                    PrintInternship(count);
                })
                .catch(error => {
                    alert('w')
        ...
        async function CreateInternship(data) {
            ...
            await fetch(`http://127.0.0.1:8000/api/internships`, {
                ...
                .then(data => {
                    ...
                    PrintInternship(0);
                    alertMessage('info', 'Please update internship data!');
        ...
        async function PrintInternship(index) {
            await fetch('http://127.0.0.1:8000/api/internships/')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
...
@push('script')
    $('.internships').children('span').removeClass('hidden');
    $('.internships').children('a').addClass('text-gray-800');
@endpush
...
@push('script')
    ...
    $('.forms > ul > li:nth-child(3)').css('color', '#9333EA');
@endpush
