            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap relative">
                        ...
                        <tbody id="partner-table-body" class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            {{-- fetch data --}}
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        ...
        function printTableBody(startCount, endCount, newData) {
            ...
            for (i; i < totalCount && i < endCount; i++) {
                ...
                $('#partner-table-body').append(`<tr row-id="${element.id}" class="text-gray-700 w-full dark:text-gray-400 partner-row">
        ...
        async function EditPartnerRow(id) {
            var keys;
            var values;
            var dataToUpdate;
            await fetch('http://127.0.0.1:8000/api/partners/' + id)
                .then(response => {
                    if (!response.ok) {
        ...
        async function DeletePartner(id) {
            ...
            await fetch(`http://127.0.0.1:8000/api/partners/${id}`, {
                ...
                .then(data => {
                    alertMessage('success', 'One partner deleted');
        ...
        async function DeletePartners(data) {
            // var data = {
            //     "title": "NOTICE HERE 4",
            //     "name": "DESCRIPTION",
            //     "date": "2024-10-13",
            ...
            await fetch(`http://127.0.0.1:8000/api/partners`, {
                ...
                .then(data => {
                    alertMessage('success', 'Selected Partners deleted!');
        ...
        async function UpdatePartner(data) {
            ...
            // "status": "/partner/id"
            // };
            await fetch(`http://127.0.0.1:8000/api/partners/${data.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
        ...
        async function CreatePartner(data) {
            ...
            await fetch(`http://127.0.0.1:8000/api/partners`, {
                ...
                .then(data => {
                    ...
                    PrintPartner(0);
                    alertMessage('info', 'Please update partner data!');
        ...
        async function PrintPartner(index) {
            await fetch('http://127.0.0.1:8000/api/partners/')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
...
@push('script')
    ...
    $('.components > ul > li:nth-child(4)').css('color', '#9333EA');
@endpush
